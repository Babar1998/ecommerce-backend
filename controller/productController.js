...
async function updatedProductController(req, res, next) {
  try {
    const {error, value} = productValidation.updatedProduct.validate(req.body)
  if(error){
    return res.send(error.details[0].message);
  } else{
    const productId = req.params.id;
    const updateProductData = req.body;
    const updated = await productService.updatedProduct(productId, updateProductData, value);
    res.send(updated);
