...
async function categoriesController(req, res, next) {
...
async function updatedCategoryController(req, res, next) {
  try {
    const {error, value} = categoryValidation.updatedCategory.validate(req.body)
  if(error){
    return res.send(error.details[0].message);
  } else{
    const categoryId = req.params.id;
    const updateCategoryData = req.body;
    const updated = await categoryService.updatedCategory(categoryId, updateCategoryData, value);
    res.send(updated);
...
async function deleteCategoryController(req, res, next) {
  try {
    const {error, value} = categoryValidation.deleteCategory.validate(req.body)
  if(error){
    return res.send(error.details[0].message);
  } else{
    const categoryId = req.params.id;
...
module.exports = {categoriesController, addCategoryController, updatedCategoryController, deleteCategoryController};